...
let rec make_let_term n : pterm =
  if n <= 0 then Var "x"
  else Let ("x", Var "x", make_let_term (n - 1))

(* Calculate size (number of nodes) *)
let rec term_size (t : pterm) = match t with
  | Var _ | N _ | Nil | Unit | Loc _ -> 1
  | Abs (_, t) | Fix t | Head t | Tail t | Ref t | Deref t -> 1 + term_size t
  | App (t1, t2) | Add (t1, t2) | Sub (t1, t2) | Cons (t1, t2) | Assign (t1, t2) -> 1 + term_size t1 + term_size t2
  | Let (_, t1, t2) -> 1 + term_size t1 + term_size t2
  | IfZero (t1, t2, t3) | IfEmpty (t1, t2, t3) -> 1 + term_size t1 + term_size t2 + term_size t3
...
let run_benchmark () =
  ...
  for i = 1 to 50 do
    let n = i * 200 in (* Depth up to 10,000 *)
    let t = make_deep_term n in
    let size = term_size t in
    let time_classic = time_f alpha_convert t in
    let time_set = time_f (fun t -> alpha_convert_with_set t StringSet.empty) t in
  ...
  for i = 10 to 18 do
    let n = i in (* Depth of tree, size grows exponentially *)
  ...
  for i = 1 to 50 do
    let n = i * 5000 in (* Depth up to 250,000 *)
