...
let get_content arg =
  if Sys.file_exists arg then read_file arg else arg

let () =
  let usage_msg = "Usage: lambda [-eval|-type] <filename|expression> or -script <filename>" in
  if Array.length Sys.argv < 3 then
    ...
    | "-eval" ->
        ...
        | Lambda.Parser.Parse_err msg -> Printf.eprintf "Parse Error: %s\n" msg
        | Failure msg -> Printf.eprintf "Evaluation Error: %s\n" msg
    ...
    | "-type" ->
        ...
        | Sys_error msg -> Printf.eprintf "File Error: %s\n" msg
        | Lambda.Parser.Parse_err msg -> Printf.eprintf "Parse Error: %s\n" msg
        | Failure msg -> Printf.eprintf "Type Error: %s\n" msg
        | e -> Printf.eprintf "An unexpected error occurred: %s\n" (Printexc.to_string e))
    | "-script" ->
        ...
        (try
          ...
          List.iter (fun t ->
            ...
            with Failure s -> print_endline ("Runtime Error: " ^ s)
               | e -> print_endline ("Runtime Exception: " ^ Printexc.to_string e)
          ) terms;
          print_endline "--------------------------------------------------"
        with
        | Sys_error msg -> Printf.eprintf "File Error: %s\n" msg
