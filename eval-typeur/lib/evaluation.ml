...
let rec rename (t : pterm) (old_var : string) (new_var : string) : pterm =
  ...
  | Abs (x, t1) ->
      if x = old_var then Abs (x, t1) else Abs (x, rename t1 old_var new_var)
  | Cons (t1, t2) ->
  ...
  | IfZero (t1, t2, t3) ->
      IfZero
        ( rename t1 old_var new_var
        , rename t2 old_var new_var
        , rename t3 old_var new_var )
  | IfEmpty (t1, t2, t3) ->
      IfEmpty
        ( rename t1 old_var new_var
        , rename t2 old_var new_var
  ...
  | Let (x, t1, t2) ->
      ...
      if x = old_var then Let (x, t1', t2)
      else Let (x, t1', rename t2 old_var new_var)
  | Unit ->
      Unit
  | Loc n ->
      Loc n
  | Ref t ->
      Ref (rename t old_var new_var)
  | Deref t ->
      Deref (rename t old_var new_var)
  | Assign (t1, t2) ->
      Assign (rename t1 old_var new_var, rename t2 old_var new_var)
...
let rec alpha_convert (t : pterm) : pterm =
  ...
  | Ref t ->
      Ref (alpha_convert t)
  | Deref t ->
      Deref (alpha_convert t)
  | Assign (t1, t2) ->
      Assign (alpha_convert t1, alpha_convert t2)
...
let rec fv (t : pterm) : StringSet.t =
  ...
  | Abs (x, t1) ->
      StringSet.remove x (fv t1)
  | Let (x, t1, t2) ->
      StringSet.union (fv t1) (StringSet.remove x (fv t2))
  | App (t1, t2) | Add (t1, t2) | Sub (t1, t2) | Cons (t1, t2) | Assign (t1, t2)
    ->
      StringSet.union (fv t1) (fv t2)
  | IfZero (t1, t2, t3) | IfEmpty (t1, t2, t3) ->
      StringSet.union (fv t1) (StringSet.union (fv t2) (fv t3))
  | Head t1 | Tail t1 | Fix t1 | Ref t1 | Deref t1 ->
      fv t1
  | N _ | Nil | Unit | Loc _ ->
      StringSet.empty
...
let rec bv (t : pterm) : StringSet.t =
  ...
  | Var _ | N _ | Nil | Unit | Loc _ ->
      StringSet.empty
  | Abs (x, t1) ->
      StringSet.add x (bv t1)
  | Let (x, t1, t2) ->
      StringSet.add x (StringSet.union (bv t1) (bv t2))
  | App (t1, t2) | Add (t1, t2) | Sub (t1, t2) | Cons (t1, t2) | Assign (t1, t2)
    ->
      StringSet.union (bv t1) (bv t2)
  | IfZero (t1, t2, t3) | IfEmpty (t1, t2, t3) ->
...
let rec alpha_convert_with_set (t : pterm) (forbidden : StringSet.t) : pterm =
  ...
  | Abs (x, t1) ->
      if StringSet.mem x forbidden then
        ...
        Abs
          ( x_new
          , alpha_convert_with_set t1_renamed (StringSet.add x_new forbidden) )
      else Abs (x, alpha_convert_with_set t1 (StringSet.add x forbidden))
  | Let (x, t1, t2) ->
      ...
      if StringSet.mem x forbidden then
        ...
        Let
          ( x_new
          , t1'
          , alpha_convert_with_set t2_renamed (StringSet.add x_new forbidden) )
      else Let (x, t1', alpha_convert_with_set t2 (StringSet.add x forbidden))
  | App (t1, t2) ->
      App
        ( alpha_convert_with_set t1 forbidden
  ...
  | Ref t1 ->
      Ref (alpha_convert_with_set t1 forbidden)
  | Deref t1 ->
      Deref (alpha_convert_with_set t1 forbidden)
  | Assign (t1, t2) ->
      Assign
        ( alpha_convert_with_set t1 forbidden
        , alpha_convert_with_set t2 forbidden )
...
let rec substitute (t : pterm) (v : string) (s : pterm) : pterm =
  ...
  | Abs (x, t1) ->
      if x = v then t else Abs (x, substitute t1 v s)
  | Cons (t1, t2) ->
  ...
  | Let (x, t1, t2) ->
      ...
      if x = v then Let (x, t1', t2) else Let (x, t1', substitute t2 v s)
  | Unit ->
      Unit
  | Loc n ->
      Loc n
  | Ref t ->
      Ref (substitute t v s)
  | Deref t ->
      Deref (substitute t v s)
  | Assign (t1, t2) ->
      Assign (substitute t1 v s, substitute t2 v s)
...
let rec beta_reduce (t : pterm) : pterm =
  ...
  | App (t1, t2) ->
      ...
      if t1' <> t1 then App (t1', t2) else App (t1, beta_reduce t2)
  | Abs (x, t1) ->
      Abs (x, beta_reduce t1)
  | Add (t1, t2) ->
      Add (beta_reduce t1, beta_reduce t2)
  | Sub (t1, t2) ->
      Sub (beta_reduce t1, beta_reduce t2)
  | Cons (t1, t2) ->
      Cons (beta_reduce t1, beta_reduce t2)
  | Head t1 ->
      Head (beta_reduce t1)
  | Tail t1 ->
      Tail (beta_reduce t1)
  ...
  | Deref t ->
      Deref (beta_reduce t)
  | Assign (t1, t2) ->
      Assign (beta_reduce t1, beta_reduce t2)
  | _ ->
      t
...
let rec is_value t =
  ...
  | Nil ->
      true
  | Cons (v1, v2) when is_value v1 && is_value v2 ->
      true
  | Unit ->
      true
  | Loc _ ->
...
let rec delta_reduce t =
  ...
  | IfEmpty (t1, t2, t3) ->
      ...
      IfEmpty (t1', t2, t3)
  (* ---- Lists ---- *)
  | Head (Cons (v, _)) when is_value v ->
      v
  | Head t1 ->
      Head (delta_reduce t1)
  | Tail (Cons (_, vs)) when is_value vs ->
      vs
  | Tail t1 ->
      Tail (delta_reduce t1)
  (* ---- Fix ---- *)
  ...
  | Deref (Loc l) -> (
    ...
    with Not_found ->
      failwith ("Segmentation fault: loc " ^ string_of_int l ^ " not found") )
  | Deref t ->
      Deref (delta_reduce t)
  | Assign (Loc l, v) when is_value v ->
      store := (l, v) :: List.remove_assoc l !store ;
      Unit
  | Assign (t1, t2) ->
      let t1' = delta_reduce t1 in
      if t1' <> t1 then Assign (t1', t2) else Assign (t1, delta_reduce t2)
  | _ ->
      t
...
let eval ?(timeout = 100) ?(trace = false) (t : pterm) : pterm =
  ...
  let rec loop t n i =
    if n <= 0 then failwith "Evaluation timeout"
    else
      let t' = delta_reduce (beta_reduce t) in
      if t' = t then t
      else (
        if trace then
          print_endline ("[" ^ string_of_int (i + 1) ^ "] " ^ print_term t') ;
        loop t' (n - 1) (i + 1) )
  in
  loop t timeout 0
