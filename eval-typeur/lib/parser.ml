...
let rec term toks =
  ...
  | LET :: VAR x :: EQUAL :: rest -> (
      ...
      | IN :: rest2 ->
          ...
          (Let (x, t1, t2), rest3)
      | _ ->
          raise (Parse_err "expected 'in' after let binding") )
  | IF :: ZERO :: rest -> (
      ...
      | THEN :: rest2 -> (
          ...
          | _ ->
              raise (Parse_err "expected 'else'") )
      | _ ->
          raise (Parse_err "expected 'then'") )
  | IF :: EMPTY :: rest -> (
      let cond, rest1 = term rest in
      match rest1 with
      | THEN :: rest2 -> (
...
and app toks =
  ...
  let rec aux acc toks =
    match toks with
    | (VAR _ | INT _ | LP | NIL | HEAD | TAIL | FIX | REF | DEREF | UNIT) :: _
      as rest ->
...
and atom = function
  ...
  | LP :: rest -> (
      let t, rest' = term rest in
      match rest' with RP :: r -> (t, r) | _ -> raise (Parse_err "missing )") )
  | _ ->
      raise (Parse_err "bad token")

let parsePTERM (s : string) : pterm =
  let t, rest = term (lex s) in
  match rest with [] -> t | _ -> raise (Parse_err "trailing tokens")
...
let rec terms toks =
  ...
  | _ -> (
      ...
      | [] ->
          [t]
      | _ ->
          raise (Parse_err "expected ;; or EOF") )

let parse_prog (s : string) : pterm list = terms (lex s)
