...
let rec appartient_type (v : string) (t : ptype) : bool =
  ...
  | Var v1 when v1 = v ->
      true
  | Arr (t1, t2) ->
      appartient_type v t1 || appartient_type v t2
  | List t1 ->
      appartient_type v t1
  | Forall (v1, t1) when v1 <> v ->
      appartient_type v t1
  | Nat ->
      false
  | Unit ->
...
let rec substitue_type (t : ptype) (v : string) (t0 : ptype) : ptype =
  match t with
  | Var x ->
      if x = v then t0 else Var x
  | Arr (t1, t2) ->
      Arr (substitue_type t1 v t0, substitue_type t2 v t0)
  | Nat ->
      Nat
  | Unit ->
      Unit
  | Ref t1 ->
      Ref (substitue_type t1 v t0)
  | List t1 ->
      List (substitue_type t1 v t0)
  | Forall (x, t1) ->
      if x = v then Forall (x, t1) else Forall (x, substitue_type t1 v t0)
...
let rec fv_type (t : ptype) : string list =
  ...
  | Var v ->
      [v]
  | Arr (t1, t2) ->
      fv_type t1 @ fv_type t2
  | List t1 ->
      fv_type t1
  | Forall (v, t1) ->
      List.filter (fun x -> x <> v) (fv_type t1)
  | Nat ->
      []
  | Unit ->
      []
...
let generalise (t : ptype) (e : env) : ptype =
  ...
  let vars_a_quantifier =
    List.filter (fun v -> not (List.mem v vars_env)) vars_t
  in
...
let substitue_type_zip (e : equa_zip) (v : string) (t0 : ptype) : equa_zip =
  match e with
  | e1, e2 ->
      (substitue_type_partout e1 v t0, substitue_type_partout e2 v t0)
...
let rec trouve_but (e : equa_zip) (but : string) =
  ...
  | _, [] ->
      raise VarPasTrouve
  | _, (Var v, t) :: _ when v = but ->
      t
  | _, (t, Var v) :: _ when v = but ->
      t
  | e1, c :: e2 ->
      trouve_but (c :: e1, e2) but

let rec unification (e : equa_zip) (but : string) : ptype =
  match e with
  | _, [] -> (
    try trouve_but (rembobine e) but
    with VarPasTrouve -> raise (Echec_unif "but pas trouvé") )
  | e1, (Var v1, t2) :: e2 when v1 = but ->
      unification ((Var v1, t2) :: e1, e2) but
  | e1, (Var v1, Var v2) :: e2 ->
      unification (substitue_type_zip (rembobine (e1, e2)) v2 (Var v1)) but
  | e1, (Var v1, t2) :: e2 ->
  ...
  | _, (t3, Ref _) :: _ ->
      raise (Echec_unif ("type ref non-unifiable avec " ^ print_type t3))
  | _ ->
      raise (Echec_unif "types non-unifiables")
...
let rec genere_equa (te : pterm) (ty : ptype) (e : env) : equa =
  ...
  | Unit ->
      [(ty, Unit)]
  | Loc _ ->
      raise (Echec_unif "Loc in source code")
  | Ref t ->
      let nv = nouvelle_var () in
