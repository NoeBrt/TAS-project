...
let rec print_term (t : pterm) : string =
  ...
  | Tail t1 ->
      "tail " ^ print_term t1
  | IfZero (t1, t2, t3) ->
      "if zero " ^ print_term t1 ^ " then " ^ print_term t2 ^ " else "
      ^ print_term t3
  | IfEmpty (t1, t2, t3) ->
      "if empty " ^ print_term t1 ^ " then " ^ print_term t2 ^ " else "
      ^ print_term t3
  | Fix t1 ->
      "fix " ^ print_term t1
  | Nil ->
...
let rec string_of_pterm (t : pterm) : string =
  ...
  | IfZero (t1, t2, t3) ->
      "IfZero (" ^ string_of_pterm t1 ^ ", " ^ string_of_pterm t2 ^ ", "
      ^ string_of_pterm t3 ^ ")"
  | IfEmpty (t1, t2, t3) ->
      "IfEmpty (" ^ string_of_pterm t1 ^ ", " ^ string_of_pterm t2 ^ ", "
      ^ string_of_pterm t3 ^ ")"
  | Fix t1 ->
  ...
  | Loc n ->
      "Loc " ^ string_of_int n
  | Ref t ->
      "Ref (" ^ string_of_pterm t ^ ")"
  | Deref t ->
      "Deref (" ^ string_of_pterm t ^ ")"
  | Assign (t1, t2) ->
      "Assign (" ^ string_of_pterm t1 ^ ", " ^ string_of_pterm t2 ^ ")"
